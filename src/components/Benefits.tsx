    ...
    <section className="py-24 relative overflow-hidden" style={{ background: "var(--hero-gradient)" }}>
      ...
      <div className="container relative z-10">
        <div className="text-center mb-16">
          ...
          <h2 className="text-3xl md:text-4xl font-bold mt-3" style={{ color: "hsl(0 0% 100%)" }}>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-3xl mx-auto">
          {benefits.map((b) => (
            <div key={b} className="flex items-center gap-3 rounded-xl bg-white/5 backdrop-blur-sm border border-white/10 px-6 py-4">
              <CheckCircle2 className="text-accent shrink-0" size={22} />
              <span className="font-medium" style={{ color: "hsl(0 0% 95%)" }}>{b}</span>
            </div>
          ))}
        </div>
      </div>
